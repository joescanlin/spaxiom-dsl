...
class MockSensor(Sensor):
    ...
    def __init__(
        self, name, privacy="public", sample_period_s=0.001, throw_error=False
    ):
        super().__init__(
            name=name,
            sensor_type="mock",
...
def test_format_sensor_value_public():
    ...
    sensor = Sensor(
        name="test_public", sensor_type="test", location=(0, 0, 0), privacy="public"
    )
    value = 42.5
...
def test_format_sensor_value_private():
    ...
    sensor = Sensor(
        name="test_private", sensor_type="test", location=(0, 0, 0), privacy="private"
    )
    value = 42.5
...
async def test_poll_sensor_privacy_error(clear_registry):
    ...
    sensor = MockSensor(
        name="private_error", privacy="private", sample_period_s=0.001, throw_error=True
    )
...
async def test_evaluate_conditions(
    clear_registry, clear_event_handlers, reset_runtime_state
):
...
@pytest.mark.asyncio
async def test_evaluate_conditions_edge_detection(
    clear_registry, clear_event_handlers, reset_runtime_state
):
...
@pytest.mark.asyncio
async def test_evaluate_conditions_exception(
    clear_registry, clear_event_handlers, reset_runtime_state
):
...
async def test_start_runtime_sensor_polling(clear_registry, reset_runtime_state):
    ...
    with patch("spaxiom.runtime._evaluate_conditions"), patch(
        "spaxiom.runtime.asyncio.Event"
    ) as mock_event, patch(
        "spaxiom.runtime.asyncio.create_task", side_effect=asyncio.create_task
    ) as mock_create_task:
...
async def test_start_runtime_signal_handlers(clear_registry, reset_runtime_state):
    ...
    with patch("spaxiom.runtime.asyncio.get_running_loop") as mock_get_loop, patch(
        "spaxiom.runtime.signal"
    ), patch("spaxiom.runtime._evaluate_conditions"), patch(
        "spaxiom.runtime.asyncio.Event"
    ) as mock_event:
...
def test_start_blocking():
    ...
    with patch("spaxiom.runtime.asyncio.run") as mock_run, patch(
        "spaxiom.runtime.sys.exit"
    ) as mock_exit:
