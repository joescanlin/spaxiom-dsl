...
def main():
    ...
    try:
        while True:
            ...
            print(
                f"[{now}] ZONE A: Pressure: {p_a:.2f}, Temp: {t_a:.1f}°C, Comfort: {c_a:.2f}",
                end="",
            )
            print(
                f" | ZONE B: Pressure: {p_b:.2f}, Temp: {t_b:.1f}°C, Comfort: {c_b:.2f}",
                end="\r",
            )
    ...
    print("=============")
    print(
        f"Zone A final readings - Pressure: {pressure_a.read():.2f}, Temperature: {thermal_a.read():.1f}°C"
    )
    print(
        f"Zone B final readings - Pressure: {pressure_b.read():.2f}, Temperature: {thermal_b.read():.1f}°C"
    )
